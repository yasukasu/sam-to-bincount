...
### Extracting fasta informatin: chro names & seq sizes

my ($fa_id_ref, $fa_n_ref) = extract_FASTA_info($file_ref);
...
if(!$ss){&aln_sort($file_aln, $file_ref, $fa_id_ref, $fa_n_ref);}
else{
    print("f: watson str\n");
    &aln_sort($file_aln1, $file_ref, $fa_id_ref, $fa_n_ref);
    print("r: Crick str\n");
    &aln_sort($file_aln2, $file_ref, $fa_id_ref, $fa_n_ref);
}
...
# outputting row names
	...
	if($chro eq $chro_name[$chro_num]){
      ...
      $total_nseq  = $total_nseq + $chro_s[$chro_num];
      $total_count = $total_count + $chro_count;

	    # moving to the next chromosome...
      $chro_num++;
      while($chro ne $chro_name[$chro_num]){
        print $chro_name[$chro_num], "\tsize:", $chro_s[$chro_num], " bp\tNo alinged read exist. => skipped.\n";
        $chro_num++;   # if empty-count chromosome exist, move to further ahead.
      }
...
#last chromsome report
&chro_report($chro_name[$chro_num], $chro_s[$chro_num],  $chro_count, $chro_non_al, $win_num, $win_x, $win_s);
$total_nseq  = $total_nseq + $chro_s[$chro_num];
$total_count = $total_count + $chro_count;

print "Total in the genome \tsize:", $total_nseq, " bp\ttotal reads:", $total_count, "\n";
...
sub help{
	print "###  Sam-to-Bincount version 1.05 ###\n";
